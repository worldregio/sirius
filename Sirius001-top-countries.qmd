...
wld <- readRDS("geom/gravitymapV5_pole.RDS")
wld<-wld %>% inner_join(param) %>% mutate(sal = mean/1000)
...
mf_map(grat, type="base", lty=3,col="gray80")
...
int <- link %>% filter(Rij <0.9999) %>% group_by(i) %>%
          mutate(rnk =rank(-Rij)) %>%
          filter(rnk <5) %>% arrange(i,rnk)
```
...
int$size<-4-int$rnk
edges <- int %>% mutate(width = 1+5*size / max(size)) %>%
...
![Network of top correlations between media coverage](img/network_correlation.jpg)

...
- Link: [https://www.scmp.com/](https://www.scmp.com/)
...
library(tidyr)
media<-"CHN_mopost"
hc<-readRDS(paste0("../sirius/",media,"/hc_int.RDS")) %>%  filter(where1 !=substr(media,1,3), where1 != "HKG")
nbnews<-sum(hc$news)
...
hctot<- readRDS("../data/hc.RDS") %>% 
      filter(where1 != substr(media,1,3), where1 != "HKG",
             where2 != substr(media,1,3), where2 != "HKG")
...
![](img/CHN_mopost_specialisation.png)

...
title: "Republika"
name: "Republika"
image: img/IDN_republ.jpg
bibliography: biblio/IDN_republ.bib
nocite: '@*'
csl: geo.csl
categories: 
- Pure player
...
library(mapsf)
media<-"IDN_republ"
hc<-readRDS(paste0("../sirius/",media,"/hc_int.RDS")) %>%  filter(! where1 %in% substr(media,1,3))
nbnews<-sum(hc$news)
...
cart<-cartogram_dorling(x=wld2,weight="sal",k = 2)
png("img/IDN_republ_salience.png",width = 1000, height=1000)
...
mf_label(cart, var = "ISO3",cex=3.5*sqrt(cart$sal/max(cart$sal)), col="white")
mf_layout("",
...
- Website : [Republika](https://www.republika.co.id/kanal/en)

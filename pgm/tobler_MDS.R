...
col <- pivot_longer(tab,cols=2:202,names_to = "i")  %>% 
          filter(i !=j) %>% 
           arrange(i,j) %>%
...
sel <- col %>% filter(Fi >99, Fj > 99, Fij>4) %>%
...
g1<- ggplot(coo) + aes(x=D1, y=D2, label=code, size = Fi,col=reg) + 
  ...
  geom_label_repel(size=2,col="black",show.legend = F) +
  scale_color_manual(values=mypal) +
  coord_fixed(ratio=1) +
  theme_light() 


ggsave(plot=g1,filename = "mds.pdf",width = 9,height=7)

g2<- ggplot(coo) + aes(x=D1, y=D2, label=code, size = Fi,col=reg) + 
  geom_point() +
